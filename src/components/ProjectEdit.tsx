    ...
    try {
      ...
      if (user) {
        await supabase.from('generation_history').insert({
          user_id: user.id,
          type: 'project_update',
          inputs: { projectId: project.id, actions: selectedActions, details: formData },
          generated_content: JSON.stringify(promptPayload, null, 2),
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {section.items.map((item) => (
                <button
                  ...
                  className={`group flex flex-col items-center justify-center py-12 rounded-xl border transition-all duration-300 relative overflow-hidden ${selectedActions.includes(item.id)
      ...
      <div className="fixed bottom-10 left-0 right-0 px-6 md:pl-[300px] md:pr-10 z-50">
        <div className="max-w-5xl mx-auto w-full">
          <button
            ...
            className={`w-full py-5 rounded-2xl font-bold flex items-center justify-center gap-3 transition-all duration-500 shadow-2xl ${selectedActions.length > 0
