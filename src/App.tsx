...
const App: React.FC = () => {
  ...
  // Scoped States (DATABASE SIMULATION)
  ...
  // 1. Check for active session on Mount and listen to changes
  useEffect(() => {
    // Check current session
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session?.user) {
        handleLogin(session.user.email!, true);
      }
    });
  ...
  // 2. Hydration Logic: Runs whenever UID changes (Login / Logout)
  useEffect(() => {
    if (!currentUid) {
    ...
    // Hydrate state from localStorage or initialize with defaults for this specific UID
    ...
    // First login audit trail
    if (!savedProfile) {
      console.log(`[Database] First time access for ${currentUid}. Initializing structure.`);
  ...
  const addProjectToList = async (data: any) => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (user) {
        ...
        if (newProj) {
          ...
          addActivity('Projeto Materializado', `A estrutura de ${project.name} foi forjada.`, 'project');
        }
      }
    } catch (error) {
  ...
  const handleLogin = (email: string, _remember: boolean) => {
  ...
  // Header só aparece na página de Visão Geral (Dashboard)
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'dashboard':
        return (
          <div className="p-6 md:px-10 md:pb-10 max-w-[1600px] w-full mx-auto relative z-10">
      ...
      case 'materializar_sas':
        return (
          <div className="p-6 md:px-10 py-10 max-w-[1600px] w-full mx-auto h-full flex flex-col relative z-10">
            {saasView === 'landing' ? (
              <SaaS
      ...
      case 'projects':
        return (
          <div className="p-6 md:px-10 py-10 max-w-[1600px] w-full mx-auto relative z-10">
            <Projects
              projects={projects}
  ...
  return (
    <div className="flex min-h-screen bg-[#050507] text-[#94A3B8] selection:bg-white/10 antialiased font-['Inter'] relative overflow-hidden">
      ...
      <main className={`flex-1 flex flex-col transition-all duration-300 ${isSidebarOpen ? 'md:ml-72' : 'ml-0'}`}>
        {showHeader && <Header userName={userData.name || userData.email || "Usuário"} />}
        {renderContent()}
      </main>
    </div>
  );
